...
function showPage(pageId, el) {
  // Hide all pages and deactivate all sidebar items
  document.querySelectorAll('.manual-page').forEach(p => p.classList.remove('active'));
  document.querySelectorAll('.sb-item').forEach(s => s.classList.remove('active'));

  // Show target page and mark sidebar item active
  const target = document.getElementById('page-' + pageId);
  if (target) target.classList.add('active');
  if (el)     el.classList.add('active');

  // On mobile, scroll the manual into view
...
// Observe everything marked for reveal
document.querySelectorAll('.reveal, .reveal-left, .reveal-right').forEach(el => {
  revealObserver.observe(el);
});

// Trigger elements already in viewport on first load
 ...
 * using an easeOutCubic easing, then appends `suffix`.
 */
...
// Watch each stat card; fire the counter once when it enters view
...
const translations = {
  en: {
    ...
    /* ── Stats ── */
    'stat.formats':   'Input formats supported',
    'stat.steps':     'Main workflow steps',
    'stat.workflows': 'n8n automation workflows',
    'stat.django':    'Django-based output',
    ...
    'ex.e1.t':     'Pokémon Catalog',
    'ex.e1.d':     'Card grid with name, type, image and stats. Filterable by type. Responsive with Tailwind.',
    'ex.e2.t':     'Weather Dashboard',
    'ex.e2.d':     'City weather cards with temperature, wind and conditions. Auto-loaded from the API on startup.',
    'ex.e3.t':     'Blog Listing',
    'ex.e3.d':     'Clean post list with title, body preview and author. Detail page per post. Ready to extend.',
    ...
    'llm.openrouter':'100+ models with one API key.',
    'llm.local':     'Ollama or LM Studio. Full privacy.',
    ...
    'p.s2.h2':    'Regenerate with a custom prompt',
    ...
    'p.s2.p1':    "If the proposal doesn't match what you want, write a natural language instruction and the LLM will regenerate the plan.",
    ...
    'p.apis.n4':        'Free API key required',
    'p.apis.n5':        'No key required',
    'p.apis.n6':        'Free API key required',
  ...
  es: {
    ...
    /* ── Stats ── */
    'stat.formats':   'Formatos de entrada soportados',
    'stat.steps':     'Pasos del flujo principal',
    'stat.workflows': 'Workflows de automatización n8n',
    'stat.django':    'Output basado en Django',
    ...
    'ex.e1.t':     'Catálogo Pokémon',
    'ex.e1.d':     'Grid de tarjetas con nombre, tipo, imagen y estadísticas. Filtrable por tipo. Responsive con Tailwind.',
    'ex.e2.t':     'Dashboard del Tiempo',
    'ex.e2.d':     'Tarjetas de clima por ciudad con temperatura, viento y condiciones. Cargado automáticamente desde la API al arrancar.',
    'ex.e3.t':     'Listado de Blog',
    'ex.e3.d':     'Lista limpia de posts con título, preview y autor. Página de detalle por post. Lista para extender.',
    ...
    'llm.groq':      'Llama 3.3 70B. La inferencia más rápida, capa gratuita.',
    'llm.openrouter':'Más de 100 modelos con una sola API.',
    'llm.local':     'Ollama o LM Studio. Privacidad total.',
    ...
    'p.s2.h2':    'Regenerar con un prompt personalizado',
    ...
    'p.s2.p1':    'Si la propuesta no encaja con lo que quieres, escribe una instrucción en lenguaje natural y el LLM regenerará el plan.',
    ...
    'p.apis.n4':        'Clave gratuita requerida',
    'p.apis.n5':        'Sin clave requerida',
    'p.apis.n6':        'Clave gratuita requerida',
 ...
 * Updates all [data-i18n] elements and saves preference to localStorage.
 */
